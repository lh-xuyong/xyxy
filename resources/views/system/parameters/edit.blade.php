...
@section('content')
    <article class="cl pd-20">
        ...
        @endif
        <form action="{{ url('system/parameters/edit',$parameter->id) }}" method="post" class="form form-horizontal">
            <input type="hidden" name="_method" value="put"/>
            {{ csrf_field() }}
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>参数名：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" class="input-text" value="{{ $parameter->name }}" placeholder="" id="username"
                           name="name">
                </div>
            </div>
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" class="input-text" name="option_list" value="@foreach($parameter->option_list as $key=>$value){{$key}}:{{$value}},@endforeach" >
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <textarea name="description" cols="" rows="" class="textarea" placeholder="说点什么...最少输入10个字符"
                              onKeyUp="textarealength(this,100)">{{ $parameter->description }}</textarea>
